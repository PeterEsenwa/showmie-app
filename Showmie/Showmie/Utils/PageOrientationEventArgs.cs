        ...
        public static void ScaleFontSizes(string Orientation)
        {
            if (Orientation == "Portrait")
            {
                if (OrientationContentPage.firstPageLoad)
                {
                    ...
                    //double diffHeight = Math.Round(height / 640, 3);
                    //double diffWidth = Math.Round(width / 360, 3);

                    //double diff = Math.Round(diffHeight / diffWidth, 2);
                    //double diff2 = Math.Round(diffWidth / diffHeight, 2);
                    //double diffOrig = Math.Round(diffHeight * diffWidth, 2);
                    //oldPortDiff = diff;
                    //OrientationContentPage.firstPageLoad = false;
                    //FontHandler.AdjustFontSizes(diff);
                    //FontHandler.AdjustFontSizes(diff2);
                    //FontHandler.AdjustFontSizes(diffOrig);
                }
                else
                {
